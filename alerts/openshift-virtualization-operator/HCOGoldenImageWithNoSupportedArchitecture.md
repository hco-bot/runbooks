...
> * For version v4.23.0 or newer, the
   `spec.workloadSources.enableMultiArchBootImageImport` field in the `HyperConverged`
   CR, is `true`.
> * For versions before v4.23.0, the
    `enableMultiArchBootImageImport` feature gate is enabled in the `HyperConverged`
CR.
## Impact
...
> Please notice:
> * In the `v1` API version, used in version v4.23.0
 or above, the `dataImportCronTemplates` field is under the `spec.workloadSources`
field.
> * In the `v1beta1` version, used in versions earlier than v4.23.0, the `dataImportCronTemplates`
field is under the `spec` field.
